...
import pencilImage from './assets/pencil.png'; // ✅ 타입 선언 추가로 해결됨
...
function App() {
      ...
      const timer = setInterval(() => {
        setUploadedFile(prev => {
          ...
          if (newProgress >= 100) {
            ...
            setTimeout(() => {
              setUploadedFile(prev =>
                prev ? { ...prev, status: 'processing', uploadProgress: 0 } : null
              );
              setTimeout(() => {
                setUploadedFile(prev =>
                  prev ? { ...prev, status: 'ready', uploadProgress: 100 } : null
                );
              }, 2000);
            }, 500);
  ...
  const handleFileUpload = async (file: PDFFile) => {
    ...
    try {
      ...
      formData.append('file', file.file); // ✅ PDFFile 타입에 file 속성 존재해야 함
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      const errorMessageContent =
        error instanceof Error ? error.message : '답변을 가져오는 중 오류가 발생했습니다.';
  ...
  return (
    <div className="min-h-screen bg-blue-50 relative overflow-hidden">
          ...
          <div className="flex items-center justify-center gap-4 mb-6">
            ...
            <div className="text-center">
              <h1
                ...
                style={{ fontFamily: 'NanumSinHonBuBu', fontSize: '38px' }}
              >
                니 필기 내꺼 ㅋ
              </h1>
              <p
                className="text-gray-600 font-bold"
                style={{ fontFamily: 'NanumSinHonBuBu', fontSize: '21px' }}
              >
                나의 비밀 요약 친구
              </p>
            </div>
      ...
      {/* History Sidebar */}
      <div
        className={`fixed top-0 left-0 h-full w-[min(90vw,480px)] bg-white transform transition-transform duration-300 ${
          showHistorySidebar
            ? 'translate-x-0 shadow-2xl'
            : '-translate-x-[calc(100%+12px)] shadow-none'
        } overflow-hidden`}
      >
