...
import { ChevronLeft, X, Send, Trash2, Star, Bookmark } from 'lucide-react'; // For back button icon
...
const HistoryPage = ({ onClose }: HistoryPageProps) => {
  ...
  useEffect(() => {
    ...
    const simulatedConversations: ConversationHistoryItem[] = [
      {
        id: 'conv-1',
        title: '이것은 매우 긴 제목을 가진 강의 노트 파일입니다. 과연 어떻게 보일까요?.pdf',
        isFavorite: false,
        uploadedFile: {
          id: 'history-pdf-1',
          name: '과거_강의노2.pdf',
  ...
  const bookStyles = [
    {
      border: 'border-l-8 border-[#a4e0e0]',
      lineColor: '#d97c7c', // Even Darker Pastel Red
    },
    {
      border: 'border-l-8 border-[#a4e0e0]',
      lineColor: '#b3b3b3', // Even Darker Pastel Gray
    },
    {
      border: 'border-l-8 border-[#a4e0e0]',
      lineColor: '#d9c57c', // Even Darker Pastel Yellow
    },
  ];
      ...
      <div className="flex-1 flex flex-col bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden"> 
        ...
        <header className="text-center py-4 border-b border-gray-100 relative">
          ...
          <h2 className="font-bold text-black" style={{ fontFamily: 'NanumSinHonBuBu', fontSize: '24px' }}>
            {selectedConversationId ? selectedConversation?.title : '나의 책장'}
          </h2>
          ...
          <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center">
            {selectedConversationId && (
              <>
                <button
                  onClick={() => handleToggleFavorite(selectedConversationId)}
                  className="p-2 rounded-full hover:bg-yellow-100 transition-colors"
                >
                  <Star className={`w-6 h-6 ${selectedConversation?.isFavorite ? 'text-yellow-500 fill-yellow-500' : 'text-gray-400 hover:text-yellow-500'}`} />
                </button>
                <button
                  onClick={() => triggerDeleteConfirmation(selectedConversationId)}
                  className="p-2 rounded-full hover:bg-red-100 transition-colors ml-1"
                >
            ...
            {onClose && (
              <button
                onClick={onClose}
                className="p-2 rounded-full hover:bg-gray-200 transition-colors ml-1"
              >
              ...
              {allConversations.map((conv, index) => {
                ...
                return (
                  <div
                    ...
                    className={`relative w-56 h-80 m-3 rounded-lg shadow-lg cursor-pointer transition-transform transform hover:scale-105 ${style.border} hover:brightness-105`}
                    style={{ backgroundColor: '#d7f5f5' }}
                  >
                    {conv.isFavorite && (
                      <Bookmark className="absolute top-[-12px] left-4 w-8 h-10 text-yellow-500 fill-yellow-500" />
                    )}
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className="absolute top-3 right-3 p-1 bg-black bg-opacity-25 rounded-full text-white hover:bg-opacity-50 transition-opacity"
                    >
                      <Trash2 className="w-4 h-4" />
                    </button>
                    <div className="pt-10">
                      <div className="w-full px-3">
                          <div className="py-2 rounded-lg" style={{ background: `linear-gradient(to left, ${style.lineColor} 12px, white 12px)` }}>
                              <h3 className={`font-bold text-[1.375rem] text-black truncate text-center px-2`} style={{ fontFamily: 'NanumSinHonBuBu' }}>{conv.title}</h3>
