...
const HistoryPage = ({ onClose }: HistoryPageProps) => {
  ...
  }, [selectedConversationId]); // Scroll when selected conversation changes
            ...
            <div className="space-y-4">
              {selectedConversation?.messages.map((message) => (
                <ChatMessage
                  key={message.id}
                  message={message}
                  uploadedFile={message.type === 'system' ? selectedConversation.uploadedFile : undefined}
                />
              ))}
                ...
                <React.Fragment key={conv.id}>
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-500">
                      {conv.messages.length} 메시지 • {new Date(conv.messages[0].timestamp).toLocaleDateString()}
                    </p>
                  </div>
